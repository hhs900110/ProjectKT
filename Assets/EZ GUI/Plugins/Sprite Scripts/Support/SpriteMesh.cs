...
public class SpriteMesh : ISpriteMesh
{
	...
	protected ControlMesh m_cotrol;
	...
	public virtual ControlMesh control
	{
		get { return m_cotrol; }
		set
		{
			...
			if(m_cotrol != null)
			{
				if(m_cotrol.spriteMesh != this)
					m_cotrol.spriteMesh = this;
			}
			else return;

			meshFilter = (MeshFilter)m_cotrol.gameObject.GetComponent(typeof(MeshFilter));
			if (meshFilter == null)
				meshFilter = (MeshFilter) m_cotrol.gameObject.AddComponent(typeof(MeshFilter));

			meshRenderer = (MeshRenderer)m_cotrol.gameObject.GetComponent(typeof(MeshRenderer));
			if (meshRenderer == null)
				meshRenderer = (MeshRenderer)m_cotrol.gameObject.AddComponent(typeof(MeshRenderer));
			...
			if (meshRenderer.sharedMaterial != null)
				...
				Debug.LogWarning("Sprite on GameObject \"" + m_cotrol.name + "\" has not been assigned a material.");
#endif
		}
	}
